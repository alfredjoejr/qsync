...
const Background = () => (
  <div className="fixed inset-0 z-[-1] overflow-hidden bg-[#0a0a1a]">
    <div
      className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full bg-indigo-600/40 blur-[120px] mix-blend-screen animate-pulse"
      style={{ animationDuration: "8s" }}
    />
    <div
      className="absolute bottom-[-20%] right-[-10%] w-[50%] h-[50%] rounded-full bg-fuchsia-600/40 blur-[120px] mix-blend-screen animate-pulse"
      style={{ animationDuration: "10s" }}
    />
    <div
      className="absolute top-[30%] right-[20%] w-[30%] h-[30%] rounded-full bg-blue-500/30 blur-[100px] mix-blend-screen animate-pulse"
      style={{ animationDuration: "7s" }}
    />
    <div
      className="absolute bottom-[20%] left-[20%] w-[40%] h-[40%] rounded-full bg-rose-500/30 blur-[100px] mix-blend-screen animate-pulse"
      ...
      <div className="glass-panel rounded-2xl px-6 py-3 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-white/80">
          {isLoggedIn && (
            <button
              ...
              className={`hover:text-white transition-colors ${currentView === "dashboard" ? "text-white font-semibold" : ""}`}
            ></button>
          )}
        </div>
        ...
        <div className="flex items-center gap-3">
          {isLoggedIn ? (
            ...
            <button
              onClick={onAuth}
              className="glass-button px-5 py-2 rounded-xl text-sm font-medium flex items-center gap-2"
            >
...
const Hero = ({ onGetStarted }: { onGetStarted: () => void }) => (
  <div className="flex flex-col items-center text-center max-w-4xl mx-auto mt-4 md:mt-8">
    ...
    >
      Book your spot, get a digital QR ticket, and receive real-time
      notifications when it's your turn. The elegant way to manage your time.
    </motion.p>
    ...
    >
      <button
        ...
        className="px-8 py-4 rounded-2xl bg-white text-black font-semibold text-lg flex items-center gap-2 hover:scale-105 transition-transform shadow-[0_0_40px_rgba(255,255,255,0.3)]"
      >
  ...
  const steps = [
    {
      ...
      title: "Select a Time",
      description:
        "Choose your preferred service, date, and morning or evening slot.",
    },
    {
      icon: <QrCode className="w-6 h-6 text-fuchsia-400" />,
      title: "Get Your QR Ticket",
      description:
        "Receive an instant digital ticket with your queue number and QR code.",
    },
    {
      icon: <Bell className="w-6 h-6 text-rose-400" />,
      title: "Smart Notifications",
      description:
        "Get reminded when you're top #5 in the queue. No more waiting in crowded rooms.",
    },
    {
      icon: <CheckCircle2 className="w-6 h-6 text-emerald-400" />,
      title: "Scan & Go",
      description:
        "Simply scan your QR code at the location to confirm your arrival.",
    },
  ];
  ...
  return (
    ...
    >
      {steps.map((step, index) => (
        <div
          key={index}
          className="glass-panel rounded-3xl p-6 hover:bg-white/10 transition-colors group"
        >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <button
            disabled={isLoading}
            className="w-full py-3 rounded-xl bg-white text-black font-semibold mt-6 hover:bg-white/90 transition-colors disabled:opacity-50"
          >
            {isLoading
              ? "Please wait..."
              : type === "login"
        ...
        <form className="space-y-4" onSubmit={handleAdminLogin}>
          <div className="space-y-1">
            ...
            <input
              ...
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="w-full glass-input rounded-xl px-4 py-3 text-sm focus:border-rose-400"
              placeholder="admin@qsync.com"
              required
            />
          </div>
          <div className="space-y-1">
          ...
          {error && (
            <p className="text-rose-400 text-sm text-center mt-2 bg-rose-500/10 py-2 rounded-lg border border-rose-500/20">
              {error}
            </p>
          )}
          ...
          <button
            disabled={isLoading}
            className="w-full py-4 rounded-xl bg-gradient-to-r from-rose-500 to-orange-500 text-white font-bold mt-6 hover:shadow-[0_0_20px_rgba(244,63,94,0.4)] transition-all disabled:opacity-50"
          >
...
const AdminDashboard = ({
  ...
  const handleDeleteUser = async (userId: number) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this user? This will also delete all their tickets.",
      )
    )
      return;
    try {
  ...
  const handleDeleteQueue = async (queueId: number) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this queue? ALL associated tickets will also be deleted!",
      )
    )
      return;
    try {
      const res = await fetch(`/api/admin/queues/${queueId}`, {
        method: "DELETE",
      });
      const data = await res.json();
  ...
  const handleAddQueue = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/admin/queues", {
        ...
        body: JSON.stringify({
          name: newQueueName,
          description: newQueueDesc,
          is_active: newQueueActive,
        }),
      });
      const data = await res.json();
      if (data.success) {
        ...
        alert(data.message || "Failed to add queue");
      }
    } catch (err) {
  ...
  const handleUpdateQueue = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/admin/queues/${editQueueData.id}`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: editQueueData.name,
          description: editQueueData.description,
          is_active: editQueueData.is_active,
        }),
      });
      const data = await res.json();
      if (data.success) {
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-6">
        ...
        <div className="glass-panel p-6 rounded-3xl flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/60 text-sm">
                Manage queues, tickets, and staff
              </p>
            </div>
          </div>
          <div className="flex items-center gap-6">
            ...
            <button
              onClick={onLogout}
              className="glass-button px-6 py-2.5 rounded-xl text-sm font-medium border-rose-500/30 hover:bg-rose-500/20 text-rose-100 transition-all"
            >
              Sign Out
            </button>
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="w-full md:w-64 flex flex-col gap-2">
            <button
              ...
              className={`text-left px-4 py-3 rounded-xl transition-all ${activeTab === "overview" ? "bg-white/10 border border-white/20 shadow-lg" : "hover:bg-white/5 text-white/70 hover:text-white"}`}
            >
              <div className="flex items-center gap-3">
                <Clock className="w-5 h-5" /> Overview
              </div>
            </button>
            <button
              onClick={() => setActiveTab("queues")}
              className={`text-left px-4 py-3 rounded-xl transition-all ${activeTab === "queues" ? "bg-white/10 border border-white/20 shadow-lg" : "hover:bg-white/5 text-white/70 hover:text-white"}`}
            >
              <div className="flex items-center gap-3">
                <Layers className="w-5 h-5" /> Manage Queues
              </div>
            </button>
            <button
              onClick={() => setActiveTab("users")}
              className={`text-left px-4 py-3 rounded-xl transition-all ${activeTab === "users" ? "bg-white/10 border border-white/20 shadow-lg" : "hover:bg-white/5 text-white/70 hover:text-white"}`}
            >
              <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`text-left px-4 py-3 rounded-xl transition-all ${activeTab === "tickets" ? "bg-white/10 border border-white/20 shadow-lg" : "hover:bg-white/5 text-white/70 hover:text-white"}`}
            >
              <div className="flex items-center gap-3">
                <QrCode className="w-5 h-5" /> Manage Tickets
              </div>
            </button>
          </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                  <div className="p-5 rounded-2xl bg-white/5 border border-white/10 flex flex-col justify-between hover:bg-white/10 transition-colors">
                    <div className="text-white/50 text-sm mb-2 flex items-center gap-2">
                      <Layers className="w-4 h-4 text-indigo-400" /> Total
                      Queues
                    </div>
                  ...
                  <div className="p-5 rounded-2xl bg-white/5 border border-white/10 flex flex-col justify-between hover:bg-white/10 transition-colors">
                    <div className="text-white/50 text-sm mb-2 flex items-center gap-2">
                      <Users className="w-4 h-4 text-fuchsia-400" /> Total Users
                    </div>
                    <div className="text-3xl font-bold">{usersList.length}</div>
                  </div>
                  <div className="p-5 rounded-2xl bg-white/5 border border-white/10 flex flex-col justify-between hover:bg-white/10 transition-colors">
                    <div className="text-white/50 text-sm mb-2 flex items-center gap-2">
                      <QrCode className="w-4 h-4 text-rose-400" /> Total Tickets
                    </div>
                    <div className="text-3xl font-bold">
                      {ticketsList.length}
                    </div>
                ...
                <div className="flex-1 bg-white/5 rounded-2xl p-6 border border-white/10 flex flex-col mt-auto">
                  <div className="flex justify-between items-center mb-8">
                    <div>
                      ...
                      <p className="text-xs text-white/50 mt-1">
                        Simulated ticket generation over the last 7 days
                      </p>
                    </div>
                  ...
                  <div className="flex-1 flex items-end justify-between gap-2 h-48 border-b border-white/10 pb-2">
                    ...
                    {[45, 60, 35, 80, 55, 90, 65].map((val, i) => (
                      <div
                        key={i}
                        className="flex flex-col items-center gap-3 w-full h-full group"
                      >
              ...
              <div>
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-white text-black text-sm font-semibold rounded-xl flex items-center gap-2 hover:bg-white/90 transition-colors"
                  >
                ...
                <div className="overflow-x-auto rounded-2xl border border-white/10">
                  <table className="w-full text-left text-sm">
                    <thead className="bg-white/5 border-b border-white/10">
                      <tr>
                        <th className="px-6 py-4 font-medium text-white/80">
                          ID
                        </th>
                        <th className="px-6 py-4 font-medium text-white/80">
                          Name
                        ...
                        <th className="px-6 py-4 font-medium text-white/80 text-right">
                          Actions
                        </th>
                      </tr>
                    </thead>
                        ...
                        queuesList.map((q) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4">
                              <span
                                className={`px-2 py-1 rounded-full text-xs font-medium border ${q.is_active ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-rose-500/10 border-rose-500/20 text-rose-400"}`}
                              >
                            ...
                            <td className="px-6 py-4 text-right">
                              ...
                              <button
                                ...
                                className="p-2 text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors inline-flex"
                              >
                    ...
                    <tbody className="divide-y divide-white/5">
                      {ticketsList.map((ticket) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-medium border ${ticket.status === "completed" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-white/10 border-white/20 text-white/80"}`}
                            >
                          ...
                          <td className="px-6 py-4 text-right">
                            <button
                              onClick={() => {
                                const formattedDate = ticket.appointment_date
                                  ? new Date(ticket.appointment_date)
                                      .toISOString()
                                      .split("T")[0]
                                  : "";
                                setEditTicketData({
              ...
              <form className="space-y-4" onSubmit={handleAddQueue}>
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full glass-input rounded-xl px-4 py-3 text-sm"
                    placeholder="e.g. Consular Services"
                    required
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    className="w-full glass-input rounded-xl px-4 py-3 text-sm"
                    placeholder="Details about this queue..."
                    rows={3}
                  />
                </div>
                <div className="flex items-center gap-3 mt-4">
                  <input
                    ...
                    onChange={(e) => setNewQueueActive(e.target.checked)}
            ...
            >
              ...
              <form className="space-y-4" onSubmit={handleUpdateQueue}>
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    value={editQueueData.description || ""}
                    onChange={(e) =>
                      setEditQueueData({
                        ...editQueueData,
                ...
                <div className="flex items-center gap-3 mt-4">
                  <input
                    type="checkbox"
                    id="editQueueActive"
                    checked={!!editQueueData.is_active}
                    onChange={(e) =>
                      setEditQueueData({
                        ...editQueueData,
...
const Dashboard = ({ userId }: { userId?: number }) => {
  ...
  const oneWeek = toLocalDateInput(
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 flex flex-col gap-2">
          <button
            ...
            className={`text-left px-4 py-3 rounded-xl transition-all ${activeTab === "new" ? "bg-white/10 border border-white/20 shadow-lg" : "hover:bg-white/5 text-white/70 hover:text-white"}`}
          >
            <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`text-left px-4 py-3 rounded-xl transition-all ${activeTab === "queue" ? "bg-white/10 border border-white/20 shadow-lg" : "hover:bg-white/5 text-white/70 hover:text-white"}`}
          >
            <div className="flex items-center gap-3">
              <Ticket className="w-5 h-5" /> My Queue
            </div>
          </button>
          <button
            onClick={() => setActiveTab("history")}
            className={`text-left px-4 py-3 rounded-xl transition-all ${activeTab === "history" ? "bg-white/10 border border-white/20 shadow-lg" : "hover:bg-white/5 text-white/70 hover:text-white"}`}
          >
            <div className="flex items-center gap-3">
              ...
              {booked ? (
                ...
                <form className="space-y-6" onSubmit={handleBooking}>
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full py-4 rounded-xl bg-white text-black font-semibold mt-4 hover:bg-white/90 transition-colors disabled:opacity-50"
                  >
                    ...
                    >
                      ...
                      <div className="w-full md:w-64 flex flex-col items-center justify-center p-6 glass-panel rounded-2xl bg-white/5">
                        ...
                        <p className="text-sm text-center text-white/60 mb-4">
                          Scan at the location to confirm arrival
                        </p>
                ...
                {tickets.filter(
                  ...
                  tickets
                    ...
                    .map((ticket) => (
                      <div
                        key={ticket.id}
                        className="glass-panel rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-white/5 transition-colors"
                      >
                        <div>
                        ...
                        <div
                          className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${ticket.status === "completed" ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}
                        >
